...
class RegionMaker(object):
    ...
    def getB(self):
        ...
        for r in range(self.pRegions):
            for i in range(len(self.data[0])):
                B[index, 0] = old_div(self.data[r][i], self.NRegion[r])
                index += 1
        return B
    ...
    def getY(self):
        ...
        for r in range(self.pRegions):
            Y += centroids[r].var * np.power(old_div(self.NRegion[r], self.N), 2)
        return Y
                    ...
                    if f == 1:
                        for move in posibleMove:
                            ...
                            if obj <= bestOBJ:
                                ...
                                totalMoves += 1
                                if (np.exp(old_div(-(obj - currentOBJ), (currentOBJ * temperature)))) > random:
                                    acceptedMoves += 1
                                    self.moveArea(area, move)
                                    self.objInfo = obj
                                    currentOBJ = obj
